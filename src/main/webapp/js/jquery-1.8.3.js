...
(function(window, undefined) {
	...
	jQuery.fn = jQuery.prototype = {
		...
		slice : function() {
			return this.pushStack(core_slice.apply(this, arguments), "slice",
	...
	jQuery
			.extend({
				...
				type : function(obj) {
					return obj == null ? String(obj) : class2type[core_toString
							.call(obj)]
							|| "object";
				},
				...
				proxy : function(fn, context) {
					...
					proxy = function() {
						return fn.apply(context, args.concat(core_slice
								.call(arguments)));
					};
			...
			.extend({
				...
				removeData : function(elem, name, pvt /* Internal Use Only */) {
					...
					if (isNode) {
						...
						// Use delete when supported for expandos or `cache` is
						// not a window per isWindow (#10080)
					} else if (jQuery.support.deleteExpando
							|| cache != cache.window) {
						delete cache[id];
				...
				acceptData : function(elem) {
					var noData = elem.nodeName
							&& jQuery.noData[elem.nodeName.toLowerCase()];

					// nodes accept data unless otherwise specified; rejection
					// can be conditional
					...
					if (elem) {
						...
						if (data) {
							if (!queue || jQuery.isArray(data)) {
								queue = jQuery._data(elem, type, jQuery
										.makeArray(data));
							} else {
								queue.push(data);
							}
	...
	if (!getSetAttribute) {
		...
		nodeHook = jQuery.valHooks.button = {
			get : function(elem, name) {
				...
				return ret
						&& (fixSpecified[name] ? ret.value !== ""
								: ret.specified) ? ret.value : undefined;
			},
			set : function(elem, value, name) {
	...
	var rformElems = /^(?:textarea|input|select)$/i, rtypenamespace = /^([^\.]*|)(?:\.(.+)|)$/, rhoverHack = /(?:^|\s)hover(\.\S+|)\b/, rkeyEvent = /^key/, rmouseEvent = /^(?:mouse|contextmenu)|click/, rfocusMorph = /^(?:focusinfocus|focusoutblur)$/, hoverHack = function(
			events) {
	...
	jQuery.event = {
		...
		dispatch : function(event) {
			...
			var i, j, cur, ret, selMatch, matched, matches, handleObj, sel, related, handlers = ((jQuery
					._data(this, "events") || {})[event.type] || []), delegateCount = handlers.delegateCount, args = core_slice
					.call(arguments), run_all = !event.exclusive
					...
					if (cur.disabled !== true || event.type !== "click") {
						...
						for (i = 0; i < delegateCount; i++) {
							...
							if (selMatch[sel] === undefined) {
								selMatch[sel] = handleObj.needsContext ? jQuery(
						...
						.add(
								...
								function(e) {
									...
									if (form
										...
										jQuery._data(form, "_submit_attached",
												true);
									}
								});
						...
						jQuery.event
								.add(this, "click._change",
										function(event) {
											...
											// Allow triggered, simulated change
											// events (#11500)
	...
	(function(window, undefined) {
		...
		operators = "([*^$|!~]?=)", attributes = "\\[" + whitespace + "*("
				+ characterEncoding + ")" + whitespace + "*(?:" + operators
				+ whitespace + "*(?:(['\"])((?:\\\\.|[^\\\\])*?)\\3|("
		...
		assertUsableName = assert(function(div) {
			// Inject content
			div.id = expando + 0;
			div.innerHTML = "<a name='" + expando + "'></a><div name='"
					+ expando + "'></div>";
			...
			if (!xml && !seed) {
				if ((match = rquickExpr.exec(selector))) {
					...
					if ((m = match[1])) {
						...
						push.apply(results, slice.call(context
								.getElementsByTagName(selector), 0));
						return results;
		...
		Sizzle.attr = function(elem, name) {
			...
			return val ? typeof elem[name] === "boolean" ? elem[name] ? name
					: null : val.specified ? val.value : null : null;
		};
						...
						: function(id, context, xml) {
							if (typeof context.getElementById !== strundefined
								...
								return m ? m.id === id
										...
										&& m.getAttributeNode("id").value === id ? [ m ]
										: undefined
										: [];
							}
						},
					...
					return pattern
							...
							&& classCache(
									...
									function(elem) {
										return pattern
												.test(elem.className
														|| (typeof elem.getAttribute !== strundefined && elem
																.getAttribute("class"))
														|| "");
									});
				},
		...
		function addCombinator(matcher, combinator, base) {
			var dir = combinator.dir, checkNonElements = base
					&& combinator.dir === "parentNode", doneName = done++;
			...
			function(elem, context, xml) {
				while ((elem = elem[dir])) {
					...
					function(elem, context, xml) {
						...
						if (!xml) {
							var cache, dirkey = dirruns + " " + doneName + " ", cachedkey = dirkey
									+ cachedruns;
							while ((elem = elem[dir])) {
								if (checkNonElements || elem.nodeType === 1) {
		...
		function select(selector, context, results, seed, xml) {
			...
			if (!seed) {
				...
				if (match.length === 1) {
					...
					for (i = matchExpr["POS"].test(selector) ? -1
							: tokens.length - 1; i >= 0; i--) {
						token = tokens[i];
			...
			// selector above
			compile(selector, match)(seed, context, xml, results,
			...
			(function() {
				...
				select = function(selector, context, results, seed, xml) {
					...
					if (!seed && !xml && !rbuggyQSA.test(selector)) {
						...
						if (newSelector) {
							try {
								push.apply(results, slice.call(newContext
										.querySelectorAll(newSelector), 0));
								return results;
							} catch (qsaError) {
							} finally {
								if (!old) {
	...
	jQuery.fn
			.extend({
				...
				closest : function(selectors, context) {
					var cur, i = 0, l = this.length, ret = [], pos = rneedsContext
							.test(selectors)
			...
			.extend({
				...
				before : function() {
					...
					if (arguments.length) {
						...
						return this.pushStack(jQuery.merge(set, this),
				...
				domManip : function(args, table, callback) {
					...
					// WebKit
					if (!jQuery.support.checkClone && l > 1
							&& typeof value === "string"
							&& rchecked.test(value)) {
						return this.each(function() {
					...
					if (this[0]) {
						...
						if (first) {
							...
							// cloned and never used in place.
	...
	function findOrAppend(elem, tag) {
		return elem.getElementsByTagName(tag)[0]
						...
						handleScript = function(elem) {
							...
							if (!elem.type || rscriptType.test(elem.type)) {
								...
								return scripts ? scripts
										.push(elem.parentNode ? elem.parentNode
						...
						for (i = 0; (elem = ret[i]) != null; i++) {
							...
							if (!(jQuery.nodeName(elem, "script") && handleScript(elem))) {
								...
								if (typeof elem.getElementsByTagName !== "undefined") {
									...
									jsTags = jQuery.grep(jQuery.merge([], elem
			...
			.extend({
				...
				css : function(elem, name, numeric, extra) {
					...
					if (numeric || extra !== undefined) {
						num = parseFloat(val);
						return numeric || jQuery.isNumeric(num) ? num || 0
								: val;
					}
					return val;
				},
	...
	if (window.getComputedStyle) {
		curCSS = function(elem, name) {
			var ret, width, minWidth, maxWidth, computed = window
					.getComputedStyle(elem, null), style = elem.style;
	...
	function setPositiveNumber(elem, value, subtract) {
		...
		return matches ? Math.max(0, matches[1] - (subtract || 0))
				+ (matches[2] || "px") : value;
	}
	...
	function css_defaultDisplay(nodeName) {
		...
		var elem = jQuery("<" + nodeName + ">").appendTo(document.body), display = elem
				.css("display");
		elem.remove();
		...
		if (display === "none" || display === "") {
			...
			elem = iframeDoc.body
					.appendChild(iframeDoc.createElement(nodeName));
	...
	if (jQuery.expr && jQuery.expr.filters) {
		jQuery.expr.filters.hidden = function(elem) {
			return (elem.offsetWidth === 0 && elem.offsetHeight === 0)
					|| (!jQuery.support.reliableHiddenOffsets && ((elem.style && elem.style.display) || curCSS(
							elem, "display")) === "none");
		};
	...
	function addToPrefiltersOrTransports(structure) {
		...
		return function(dataTypeExpression, func) {
			...
			var dataType, list, placeBefore, dataTypes = dataTypeExpression
					.toLowerCase().split(core_rspace), i = 0, length = dataTypes.length;
	...
	function inspectPrefiltersOrTransports(structure, options, originalOptions,
		...
		var selection, list = structure[dataType], i = 0, length = list ? list.length
				: 0, executeOnly = (structure === prefilters);
		...
		jQuery.ajax(
				{
					...
					// if "type" variable is undefined, then "GET" method will
					// be used
					type : type,
					dataType : "html",
					data : params,
	...
	var oldCallbacks = [], rquestion = /\?/, rjsonp = /(=)\?(?=&|$)|\?\?/, nonce = jQuery
			.now();
			...
			.ajaxPrefilter(
					...
					function(s, originalSettings, jqXHR) {
						...
						if (s.dataTypes[0] === "jsonp" || replaceInUrl
							...
							if (replaceInUrl) {
								s.url = url
										.replace(rjsonp, "$1" + callbackName);
							} else if (replaceInData) {
								s.data = data.replace(rjsonp, "$1"
										+ callbackName);
							} else if (hasCallback) {
					...
					if (!s.crossDomain || jQuery.support.cors) {
						...
						return {
							send : function(headers, complete) {
								...
								if (!s.async) {
									// if we're in sync mode we fire the
									// callback
									callback();
	...
	var fxNow, timerId, rfxtypes = /^(?:toggle|show|hide)$/, rfxnum = new RegExp(
		...
		"*" : [ function(prop, value) {
			var end, unit, tween = this.createTween(prop, value), parts = rfxnum
					.exec(value), target = tween.cur(), start = +target || 0, scale = 1, maxIterations = 20;
		...
		jQuery
				.each(
						...
						function(prop, value) {
							...
							for (; index < length; index++) {
								if (collection[index].call(animation, prop,
										value)) {
	...
	function Animation(elem, properties, options) {
		var result, index = 0, tweenerIndex = 0, length = animationPrefilters.length, deferred = jQuery
			...
			function(i, name) {
				...
				jQuery.fn[name] = function(speed, easing, callback) {
					return speed == null
							...
							// )
							(!i && jQuery.isFunction(speed) && jQuery
									.isFunction(easing)) ? cssFn.apply(this,
							arguments) : this.animate(genFx(name, true), speed,
							easing, callback);
				};
			});
